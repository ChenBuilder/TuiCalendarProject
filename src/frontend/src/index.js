...
trigger.addEventListener("click", () => {
  ...
  const apiUrl = `http://34.85.98.65/api/groups`;
    ...
    .then((data) => {
      data.forEach(group => {
        ...
        anchor.addEventListener('click', function() {
          ...
          const groupeEventUrl = `http://34.85.98.65/api/groupsevents?groupEmail=${groupName}&minTime=${startDate}&maxTime=${endDate}`;
...
function displayEvents(){
  ...
  const [startDate, endDate] = getLocaltTime();
  const apiUrl = `http://34.85.98.65/api/events?minTime=${startDate}&maxTime=${endDate}`; 
